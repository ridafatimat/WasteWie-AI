...
def normalize_product_name(value: str) -> str:
    """Create a stable matching key across pantry, meal, and receipt names."""
...
def _stock_and_metadata(
    db: Session,
    household_id: str,
) -> tuple[dict[tuple[str, str], dict[str, Any]], dict[tuple[str, str], date]]:
    ...
    for item in pantry_items:
        base_initial, base_unit = canonicalize_quantity(
            item.product_name, item.quantity_initial, item.unit
        )
        key = (normalize_product_name(item.product_name), base_unit)
        if key not in earliest_purchase or item.purchase_date < earliest_purchase[key]:
            earliest_purchase[key] = item.purchase_date

        if item.status != PantryStatus.active or item.quantity_remaining <= 0:
            continue

        base_remaining, _ = canonicalize_quantity(
            item.product_name, item.quantity_remaining, item.unit
        )
...
def _consumption_requirements(
    ...
    history_start = datetime.combine(
        today - timedelta(days=MAX_HISTORY_DAYS - 1), datetime.min.time()
    ...
    for event, item in rows:
        ...
        base_quantity, base_unit = canonicalize_quantity(
            item.product_name, consumed_quantity, item.unit
        )
        key = (normalize_product_name(item.product_name), base_unit)
        ...
        if record["first_event_date"] is None or occurred_date < record["first_event_date"]:
            record["first_event_date"] = occurred_date

    # Cold-start and backward-compatibility fallback:
    # infer usage from quantity_initial - quantity_remaining when older clients
    # changed pantry quantities without creating explicit consumed events.
    snapshot_rows = (
        db.query(PantryItem)
        .filter(
            PantryItem.household_id == household_id,
    ...
    for key, data in grouped.items():
        start_candidates = [d for d in (data["first_event_date"], earliest_purchase.get(key)) if d]
        observation_start = max(min(start_candidates), today - timedelta(days=MAX_HISTORY_DAYS - 1))
        observed_days = max(MIN_OBSERVATION_DAYS, (today - observation_start).days + 1)
        ...
        confidence = min(
            0.95,
            0.40 + min(data["event_count"], 5) * 0.08 + min(observed_days, 30) * 0.01,
        )
        requirements[key] = {
            "product_name": stock.get(key, {}).get("product_name") or data["product_name"],
            "category": stock.get(key, {}).get("category") or data["category"],
            "required": required,
...
def _meal_requirements(grocery_list: GroceryList) -> dict[tuple[str, str], dict[str, Any]]:
    requirements: dict[tuple[str, str], dict[str, Any]] = {}
    ...
    for meal in grocery_list.meal_plans:
        for ingredient in meal.ingredients or []:
            ...
            key = (normalize_product_name(name), base_unit)
...
def _make_recommendations(
    ...
    stock, earliest_purchase = _stock_and_metadata(db, household_id)
    consumption = _consumption_requirements(
        db,
        household_id,
    ...
    meals = _meal_requirements(grocery_list)
    keys = set(consumption) | set(meals)
    ...
    for key in keys:
        ...
        current_stock = stock.get(key, {}).get("quantity", 0.0)
        consumption_required = consumption_data["required"] if consumption_data else 0.0
        meal_required = meal_data["required"] if meal_data else 0.0
        total_required = consumption_required + meal_required
        purchase_base = _round_purchase_base(total_required - current_stock, base_unit)
        ...
        average_daily = (
            consumption_data["average_daily"] / divisor if consumption_data else None
        )
        estimated_days = consumption_data["estimated_days"] if consumption_data else None

        if consumption_data and meal_data:
        ...
        if consumption_data and estimated_days is not None and estimated_days <= 3:
            priority = "buy_soon"
        elif meal_data and not consumption_data:
        ...
        if consumption_data:
            ...
            if consumption_data["inferred_from_snapshot"]:
                reason_parts.append(
                    f"Estimated household use from pantry quantity changes is about "
                    f"{_round_number(average_daily, 2)} {display_unit}/day."
                )
            else:
                reason_parts.append(
...
def regenerate_grocery_list(
    ...
    for item in list(grocery_list.items):
        ...
        existing_by_key[(normalize_product_name(item.product_name), base_unit)] = item
...
def reconcile_receipt_changes(
    ...
    for item in grocery_list.items:
        _, base_unit = canonicalize_quantity(item.product_name, 1, item.unit)
        items_by_key[(normalize_product_name(item.product_name), base_unit)] = item
    ...
    for change in pantry_changes:
        ...
        key = (normalize_product_name(product_name), base_unit)
