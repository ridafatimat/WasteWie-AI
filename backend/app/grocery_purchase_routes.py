...
def repair_missing_pantry_metadata(
    db: Session,
    household_id: str,
) -> int:
    """
    Fill missing storage and expiry values for active pantry batches.

    This repairs historical grocery-shopping purchases that were previously
    saved with storage_location='unknown' and expiry_date=None. Existing valid
    ...
    pantry_items = (
        ...
        .filter(
            ...
            or_(
                ...
                PantryItem.storage_location.is_(None),
                func.lower(
                    func.trim(PantryItem.storage_location)
                ).in_(("", "unknown", "none")),
            ),
        )
        .all()
    ...
    for pantry_item in pantry_items:
        ...
        current_category = _category_value(
            pantry_item.category
        )

        if (
            current_category == "other"
            and inferred_category != "other"
        ):
            pantry_item.category = inferred_category
            changed = True

        current_storage = str(
            pantry_item.storage_location or ""
        ).strip().lower()

        if current_storage in {
            "",
            "unknown",
            "none",
        }:
            pantry_item.storage_location = (
                inferred_storage
            )
            changed = True

        if pantry_item.expiry_date is None:
            pantry_item.expiry_date = inferred_expiry
            changed = True
...
def return_grocery_list_to_draft(
    ...
    repair_missing_pantry_metadata(
        db,
        household_id,
    )
...
def mark_grocery_item_purchased(
    ...
    # Shopping-mode purchases also create a pending household ML sample.
    from .ml import ensure_training_sample
