"""Train and evaluate WasteWise's global offline waste-risk classifier.

The global Kaggle model remains the default/fallback model. Household-specific
models are created later by app.ml using this same normalized feature space.

Examples:
  python training/train.py --data training/food_expiry_tracker.csv
  python training/train.py --data training/food_expiry_tracker.csv --model xgboost
"""
...
BOOLEAN_COLUMNS = [
    ...
    if column.startswith(("item_", "storage_"))
]
MODEL_VERSIONS = {
...
def load_training_data(data_path: Path) -> tuple[pd.DataFrame, pd.Series]:
    ...
    for column in BOOLEAN_COLUMNS:
        if frame[column].dtype == object:
            ...
            if invalid.any():
                ...
                raise ValueError(
                    f"Column '{column}' contains invalid boolean values: {examples}"
                )
    ...
    numeric_columns = [
        column for column in FEATURE_COLUMNS if column not in BOOLEAN_COLUMNS
    ]
    ...
    if (
        not set(source_labels.unique()).issubset({0, 1})
        or source_labels.nunique() != 2
    ):
        raise ValueError(
            "'used_before_expiry' must contain both binary outcomes 0 and 1"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to food_expiry_tracker.csv",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("artifacts/waste_risk_model.joblib"),
        help="Where to save the global model artifact",
    )
    parser.add_argument(
        "--model",
